...
export default function ContactoClient() {
  ...
  const goToSlide = useCallback((index: number, direction: number = 0) => {
    ...
    const prevSlide = document.getElementById(`contacto-slide-${prevIndex}`);
    const nextSlide = document.getElementById(`contacto-slide-${nextIndex}`);
    ...
    const nextBg = nextSlide.querySelector(
      ".contact-layout, .footer-quote-inner"
    );
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-grow swipe-container" style={{ background: "var(--black)" }}>
        ...
        {/* Slide 1: Cierre / Footer */}
        <div className="swipe-section" id="contacto-slide-1">
          <div className="outer">
            <div className="inner">
              <Footer onNavigate={(id) => handleNavigate(id, 0)} />
            </div>
          </div>
        </div>
      </main>
